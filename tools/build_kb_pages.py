...
/var/www/ai4bcm-demo/kb/<id>/index.html plus a kb/index.html listing. Served by
...
uses are supported (bold, italic, inline code, links, ul, ol, blockquote, continuation
lines, fenced blocks and pipe tables) — headings appear in 0/96 chunks, so they are
not parsed. Tables, fences and links joined the list on 2026-09-10, when the corpus
became the guidance units: 24 of the 96 chunks carry a table and 10 a prompt fence.
"""
...
OWN_SOURCE_ID = "ai4bcm-guidance"


def _attribution(chunk: dict, sources: dict) -> str:
    """The attribution block for a carried third-party source; "" for our own text.

    Every open licence in this corpus is conditional. Decision 2011/833/EU and the Open
    Government Licence require acknowledgement of source; CC BY additionally requires that
    changes be indicated. Every carried file IS a change -- a PDF or XHTML turned into
    Markdown and re-chunked -- so the conversion notice discharges a licence term rather than
    decorating the page. Citation and licence ride on the chunk; publisher, source URL and the
    conversion line come from the registry row.
    """
    sid = (chunk.get("source_id") or "").strip()
    if not sid or sid == OWN_SOURCE_ID:
        return ""
    row = sources.get(sid, {})
    e = html.escape
    out = ['<div class="attrib"><b>Source and licence</b>']
    if chunk.get("citation"):
        out.append("<p>%s</p>" % e(str(chunk["citation"])))
    pub, url = row.get("publisher"), row.get("source_url")
    if pub and url:
        out.append('<p>Published by %s at <a href="%s">%s</a>.</p>'
                   % (e(str(pub)), e(str(url), quote=True), e(str(url))))
    elif pub:
        out.append("<p>Published by %s.</p>" % e(str(pub)))
    if chunk.get("licence"):
        out.append("<p>Reuse terms: %s</p>" % e(str(chunk["licence"])))
    if row.get("conversion"):
        out.append("<p><b>Changes indicated</b>%s The original linked above is authoritative; "
                   "this page is a conversion.</p>" % (" " + e(str(row["conversion"]))))
    out.append("</div>")
    return "\n".join(out)


def load_sources(path) -> dict:
    """Registry rows by id. Missing file is not fatal: pages still build, without attribution."""
    try:
        rows = json.loads(Path(path).read_text(encoding="utf-8")).get("sources", [])
...
.meta a.unit{color:var(--paper);background:var(--accent);border:0;border-radius:var(--r-pill);
padding:.22rem .62rem;font-weight:700}
.meta a.unit:hover{background:var(--accent-2);color:var(--paper)}
...
.tw{overflow-x:auto;margin:0 0 1.15rem}
table{border-collapse:collapse;width:100%;font-size:var(--t-body-sm);line-height:var(--lh-body-sm)}
th,td{text-align:left;vertical-align:top;padding:.55rem .7rem;border-bottom:1px solid var(--rule)}
th{font:700 .78rem/1.5 var(--sans);letter-spacing:.02em;text-transform:uppercase;color:var(--ink-3);
border-bottom:1px solid var(--ink-3)}
pre{overflow-x:auto;margin:0 0 1.15rem;padding:1rem 1.1rem;background:var(--tint);
border:1px solid var(--rule);border-radius:8px}
pre code{font:.84rem/1.6 var(--mono);background:none;border:0;padding:0;white-space:pre}
a{color:var(--accent);text-underline-offset:.18em;text-decoration-thickness:1px}
a:hover{color:var(--accent-2)}
:focus-visible{outline:2px solid var(--accent);outline-offset:3px;border-radius:6px}
.attrib{margin:2.2rem 0 0;padding:.9rem 1rem;border:1px solid var(--rule);border-radius:4px;
font-size:var(--t-caption);line-height:1.5;color:var(--ink-2)}
.attrib b{display:block;margin-bottom:.35rem}
.attrib p{margin:.3rem 0}
...
# Unit slug → the heading its group carries on the index. A unit missing from this map
# falls back to its slug, so a new unit lands readably without an edit here.
UNIT_LABEL = {
    "all": "Across the guidance",
    "principles": "Principles", "levels": "Where you stand", "data-rules": "Data rules",
    "tools": "Tools", "workflow-design": "Workflow design", "glossary": "Glossary",
    "references": "References and further reading",
    "stages-govern": "Establishing and governing BCM",
    "stages-embed": "Embedding BCM in the organisation",
    "stages-analysis": "Analysis: business impact and risk",
    "stages-design": "Continuity strategies and solutions design",
    "stages-implement": "Implementing continuity arrangements",
    "stages-validate": "Validation, exercising, review and improvement",
    "prompts-readme": "Prompts: the pattern", "prompts-draft": "Prompt: drafting",
    "prompts-review": "Prompt: review and challenge", "prompts-bia": "Prompt: BIA support",
    "prompts-awareness": "Prompt: awareness content",
    "prompts-exercise": "Prompt: exercise scenario",
    "prompts-management-report": "Prompt: management reporting",
    "prompts-evaluations": "Prompt evaluations",
}
...
def render_markdown(text: str, title: str = "", breadcrumb: str = "") -> str:
    ...
    quote: list[str] = []
    rows: list[str] = []   # pipe-table rows, flushed when the table ends
    fence: list[str] | None = None   # inside a ``` block when not None
    first_para = True
    ...
    def close_table() -> None:
        """A pipe table: first row is the header, the dashes row is the separator.

        Wrapped in a scroller, because the selection guide is five columns wide and a
        phone must not be handed a horizontally scrolling page to read one table.
        """
        if not rows:
            return
        cells = [[c.strip() for c in r.strip().strip("|").split("|")] for r in rows]
        rows.clear()
        head = cells[0] if len(cells) > 1 and all(
            set(c) <= set("-: ") for c in cells[1]) else []
    ...
    for raw in lines:
        ...
        # A fenced block is verbatim: the six-element prompt pattern is the artefact a
        # reader copies, so nothing inside it is parsed or marked up.
    ...
    close_quote()
    close_table()
    if fence is not None:   # unterminated fence: keep the text rather than drop it
        out.append("<pre><code>" + html.escape("\n".join(fence)) + "</code></pre>")
    return "\n".join(out)
...
def _rail(c: dict) -> str:
    ...
    is a working link: the unit jumps to its index group, the rest open facet pages."""

    parts: list[str] = []
    unit = (c.get("unit") or "").strip()
    if unit:
        parts.append(f'<a class="unit" href="../#{html.escape(unit)}">'
                     f'{html.escape(UNIT_LABEL.get(unit, unit))}</a>')
...
def build(chunks_path: Path = DEFAULT_CHUNKS, out_dir: Path = DEFAULT_OUT,
          sources_path: Path | None = None) -> int:
    sources = load_sources(sources_path or (Path(__file__).resolve().parent / "sources.json"))
    ...
    for c in chunks:
        ...
        (page_dir / "index.html").write_text(
            PAGE.format(style=STYLE, title=html.escape(c["title"]),
                        ...
                        rail=_rail(c),
                        attrib=_attribution(c, sources),
                        body=render_markdown(text, c["title"], c["breadcrumb"])),
            encoding="utf-8")
        groups.setdefault((c.get("unit") or "all").strip() or "all", []).append(
            (c["breadcrumb"], c["id"]))
...
if __name__ == "__main__":
    ...
    ap.add_argument("--sources", type=Path, default=Path(__file__).resolve().parent / "sources.json",
                    help="the source registry; supplies publisher, source URL and the conversion notice")
    ap.add_argument("--chunks", type=Path, default=DEFAULT_CHUNKS)
    ap.add_argument("--out", type=Path, default=DEFAULT_OUT)
    args = ap.parse_args()
    n = build(args.chunks, args.out, args.sources)
